...
class AdminBlogComposeScreen extends StatefulWidget {
  ...
  @override
  State<AdminBlogComposeScreen> createState() => _AdminBlogComposeScreenState();
}
...
class _AdminBlogComposeScreenState extends State<AdminBlogComposeScreen>
  ...
  Future<void> _save({required String status}) async {
    ...
    final payload = <String, dynamic>{
      ...
      'content': rawHtml,
      'status': status,
      if (_excerptCtrl.text.isNotEmpty) 'excerpt': _excerptCtrl.text.trim(),
      if (_categoriesCtrl.text.isNotEmpty)
        'categories': _splitComma(_categoriesCtrl.text),
      if (_tagsCtrl.text.isNotEmpty) 'tags': _splitComma(_tagsCtrl.text),
      ...
      if (_cityIdCtrl.text.isNotEmpty) 'cityId': _cityIdCtrl.text.trim(),
        ...
        if (slug.isNotEmpty) {
          await FirebaseFirestore.instance
              ...
              .set(
            {
              ...
              'categories':
                  payload['categories'] ?? savedPost['categories'] ?? [],
              'tags': payload['tags'] ?? savedPost['tags'] ?? [],
              'featuredImage':
                  payload['featuredImage'] ?? savedPost['featuredImage'],
              'metaTitle': payload['metaTitle'] ?? savedPost['metaTitle'],
              'metaDescription':
                  payload['metaDescription'] ?? savedPost['metaDescription'],
...
class _SettingsTab extends StatelessWidget {
  ...
  Widget _statusOption(
      String value, IconData icon, Color color, ValueChanged<String> onChange) {
