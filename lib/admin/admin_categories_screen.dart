...
//
// Key differences from the old channels screen:
//   • No city selector — categories are NOT city-scoped.
//   • Subcategories are real DB records, not a text string.
//   • Each category has a slug, emoji icon, sortOrder, and isActive flag.
// ─────────────────────────────────────────────────────────────────────────────
...
class _AdminCategoriesScreenState extends State<AdminCategoriesScreen> {
  ...
  Future<void> _deleteCategory(CategoryModel cat) async {
    ...
    if (hasChildren) body = 'This will also delete all subcategories. Use cascade delete.';
    if (hasLinks) body = 'This category is linked to ${cat.placeLinksCount} place(s). Remove links first.';
          ...
          Row(children: [
            const Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('Global service categories — shared across all cities',
                    style: TextStyle(color: Colors.white38, fontSize: 12)),
              ]),
            ),
            AdminAddButton(label: 'Add Category', onTap: () => _openRootForm()),
          ]),
  ...
  Widget _buildBody() {
    ...
    if (roots.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('Create your first category like Accommodation, Dining, or Wellness.',
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) => _RootCategoryCard(
        ...
        onEditSubcategory: (child) => _openSubcategoryForm(roots[i], existing: child),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              _CountPill(
                  ...
                  value: '${category.children.isNotEmpty ? category.children.length : category.childrenCount}',
                  label: 'subcats'),
              const SizedBox(width: 8),
              _CountPill(
              ...
              child: GestureDetector(
                ...
                child: Row(children: [
                  Icon(Icons.add_circle_outline_rounded,
                      ...
                      size: 16),
                  const SizedBox(width: 8),
                  Text('Add subcategory to ${category.name}',
                      style: TextStyle(
...
class _ChildCategoryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        padding: const EdgeInsets.fromLTRB(62, 2, 16, 2),
        child: Row(children: [
          Container(
            width: 6, height: 6,
            decoration: BoxDecoration(
          ...
          const SizedBox(width: 8),
          IconButton(
            icon: const Icon(Icons.edit_rounded, color: Colors.white38, size: 15),
            onPressed: onEdit,
            tooltip: 'Edit',
          ...
          IconButton(
            icon: const Icon(Icons.delete_rounded, color: Colors.redAccent, size: 15),
            onPressed: onDelete,
            tooltip: 'Delete',
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(minWidth: 28, minHeight: 28),
...
// ─────────────────────────────────────────────────────────────────────────────
// Category Form Dialog — handles both root and subcategory creation/editing
// ─────────────────────────────────────────────────────────────────────────────
...
class _CategoryFormDialogState extends State<_CategoryFormDialog> {
  ...
  @override
  void dispose() {
    _name.dispose(); _slug.dispose(); _icon.dispose();
    _description.dispose(); _sortOrder.dispose();
    super.dispose();
  }

  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) return;
    setState(() { _saving = true; _apiErrors = null; });
    ...
    try {
      ...
      if (mounted) {
        setState(() { _apiErrors = e.errors; _saving = false; });
    ...
    return AdminDialog(
      ...
      child: Form(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Parent category selector (only for creating subcategories or reassigning)
...
class _CountPill extends StatelessWidget {
  ...
  final String value;
  final String label;
  const _CountPill({required this.icon, required this.value, required this.label});
